...
<?php
if(isset($_GET['id'])){
	$qry = $conn->query("SELECT * FROM instructor where id=".$_GET['id'])->fetch_array();
	foreach($qry as $k =>$v){
		$$k = $v;
	}
}
...
<div class="container-fluid">
	<form action="" id="manage-instructor">
		<div id="msg"></div>
...
<script>
	$('#manage-instructor').submit(function(e){
		e.preventDefault()
		start_load()
		$.ajax({
			url:'ajax.php?action=save_instructor',
			method:'POST',
			data:$(this).serialize(),
